...
const DeliveryPartnerSignup = () => {
    const [input, setInput] = useState({
        ...
        address: '', city: '', state: '', vehicleType: 'bike', vehicleNumber: ''
    })
    ...
    const handleSubmit = async (e) => {
        ...
        if (!input.name.trim() || !input.email.trim() || !input.phone || !input.password.trim() || 
            !input.cpassword.trim() || !input.address.trim() || !input.vehicleNumber.trim()) {
        ...
        let response = await fetch("/api/delivery/signup", {
            method: "POST",
            headers: {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 py-4 px-2 sm:px-4">
            <div className="w-full max-w-5xl bg-white rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col lg:flex-row lg:min-h-[90vh]">
                {/* Left Side - Branding */}
                ...
                {/* Right Side - Form */}
                <div className="w-full lg:w-1/2 p-4 sm:p-6 md:p-8 flex flex-col justify-center overflow-y-auto max-h-screen lg:max-h-full">
                    <div className="mb-4 sm:mb-6">
                        <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-1 sm:mb-2">
                            Become a Partner
                        </h2>
                        <p className="text-sm sm:text-base text-gray-600">
                            Register to start earning with us
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                            <div>
                                <label htmlFor="vehicleType" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">
                                    Vehicle Type
                                </label>
                                <select
                                    id="vehicleType"
                            ...
                            <div>
                                <label htmlFor="vehicleNumber" className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">
                                    Vehicle Number
                                </label>
                                <input
                                    id="vehicleNumber"
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                rows="2"
                                required
                                className="w-full px-3 sm:px-4 py-2 sm:py-2.5 text-sm sm:text-base border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition resize-none"
                        ...
                        <div className="text-center pt-1 sm:pt-2">
                            <p className="text-xs sm:text-sm text-gray-600">
                                ...
                                <Link href="/delivery-auth/login" className="font-semibold text-blue-600 hover:text-blue-700">
                                    Sign In
                                </Link>
                            </p>
                        </div>
